...
class GS:
    def __init__(self, message, dictionary, ops, threshold):
        ...
        self.expanded = []
        self.fringe = deque()
        self.fringe.append((message, [], 0, self.task5(message)))
        self.ops = ops
    ...
    def find_neighbors(self, message, keys, depth):
        ...
        for k in self.ops: # loop through all operators
            ...
            keys_curr.append(k)
            neighbors.append((dec, keys_curr, depth + 1, self.task5(dec, false)))
        return neighbors
    
    def sort_fringe(self):
        self.fringe = deque(sorted(self.fringe, key=lambda x: x[3]))
        return
                           
    def task5(message, is_goal):
      if is_goal:
        return 0
      else:
